//
// Unified Logging for Android and Desktop
//
...
static inline void llama_log_callback(enum ggml_log_level level,
                                      const char* text,
                                      void* /*user*/) {
...
static inline void llama_log_callback(ggml_log_level level, const char * text, void * user_data) {
    (void) level;
    (void) user_data;
    fputs(text, stdout);
    fflush(stdout);
